...
public abstract class AbstractLayer {
	 ...
	 */
	public AbstractLayer(Header h, IAudio audio, Mp3ToWav mp3ToWav) {
	...
	protected int outputPCM() {
		synchronized (audioLock) {
			...
			while (remainder > 0) {
				...
				else if (mp3ToWav != null)
				{
					if ((w = mp3ToWav.write(pcmbuf, readCursor, remainder)) == 0)
						return 0;
				}
				remainder -= w;
			...
			if (audio != null)
			{
				w = audio.write(pcmbuf, readCursor, size);
			}
			else if (mp3ToWav != null)
			{
				w = mp3ToWav.write(pcmbuf, readCursor, size);
			}
			else 
			{
				w = size;
	...
	protected void close(boolean interrupted) {
		...
		if (mp3ToWav != null && started()) {
			if (!interrupted) {
				if(writeCursor[0] > readCursor)
					mp3ToWav.write(pcmbuf, 0, writeCursor[0] - readCursor);
			}
			mp3ToWav.close();
		}
